...
interface AttemptActions {
  ...
  startAttempt: (data: StartAttemptRequest, faceImage?: { dataUrl: string; name: string }) => Promise<void>;
    ...
    (set, get) => ({
      ...
      fetchMyAttempts: async () => {
        ...
        try {
          ...
          set({ error: error.message || 'Failed to fetch attempts', isLoading: false });
        }
      },
      ...
      fetchAttemptById: async (id: string) => {
        ...
        try {
          ...
          set({ error: error.message || 'Failed to fetch attempt', isLoading: false });
        }
      },

      startAttempt: async (data: StartAttemptRequest, faceImage?: { dataUrl: string; name: string }) => {
        set({ isLoading: true, error: null });
        try {
          const attempt = await attemptsApi.startAttempt(data, faceImage);
          set({ currentAttempt: attempt, isLoading: false });
        } catch (error: any) {
          set({ error: error.message || 'Failed to start attempt', isLoading: false });
          throw error;
        }
      },
      ...
      submitAttempt: async (id: string, data: SubmitAttemptRequest) => {
        ...
        try {
          ...
          if (get().isOffline) {
            ...
            set({ error: error.message || 'Failed to submit attempt', isLoading: false });
            throw error;
          }
        }
      },
